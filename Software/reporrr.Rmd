...
minus_res3 = 0
 ...
 value <- c( data_to_report()$R, data_to_report()$well_stor, data_to_report()$well_depth, data_to_report()$additional_resistance, additional_from_slope(), data_to_report()$skin, data_to_report()$Cbar)
      units <- c("[m]",  "[-]", "[m]", "[m]", "[m]",  "[-]",  "[-]")
...
```{r , eval = raw, collapse=TRUE, echo=FALSE,comment = NA, fig.width = 11, fig.asp = 0.5, fig.align ='left', warning = FALSE }
 ...
 plot_raw <- plot_grid(ggplot(data_to_report()$data_ok, aes(x = time,
     ...
     labs(color = "") + 
                       theme(text = element_text(size = 18)), scale = 1)
...
```{r , eval = graph_res, collapse=TRUE, echo=FALSE,comment = NA, fig.width = 11, fig.asp = 0.5, fig.align ='left', warning = FALSE }
