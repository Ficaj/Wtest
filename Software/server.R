...
function(input, output, session) {
 ...
 output$dir_select <- renderUI({
   if(miss_input() == 0)
      return(shinyDirButton("dir", "Chose directory", title = "", class = "btn-primary"))

   else
     return(shinyDirButton("dir", "Chose directory", title = "", class = "btn-warning"))
    })
  ...
  observeEvent(c(input$file1, input$sep, input$dec, input$length_data1x, input$length_data1t), {

    if (is.null(input$file1) ||
        is.na(data_to_report()$file_t) ||
        is.null(input$sep) || is.null(input$dec) || format_file1() == 1)
      return ()
  ...
  observeEvent(input$load_sheet, {
    if (is.null(input$file1) ||
        ...
        is.null(input$sep) || is.null(input$dec) || is.null(input$sheet))
      return ()
  ...
  observeEvent(c(time_col(), drawdown_col(), data()), {
    ...
    if (is.null(data_to_report()$time_col) ||
      ...
      if (new_or_load() == 0) {
        dat <- data.frame(time = data()[, time_col()], drawdown = data()[, drawdown_col()])
  ...
  updateCheckboxGroupInput(session, inputId = "report_contain",selected = c("Include",
                                                         "Graph - input data",
                                                         "Graph - data used",
                                                         "Graph - fitted values"))
  })
  ...
  output$Trans <- renderUI({
    numericInput(
      inputId = "Trans",
      label = NULL,
      if (exists("setupWT") && new_or_load() == 1 || slider_tr() == "yes") {
  ...
  observeEvent(c(data(), input$file1, input$sep, input$dec, input$length_data1x, input$length_data1t), {
    ...
    input$previous_eval_p),
  {
        ...
        is.null(max_val()))
    {
      ...
      output$graph1 <- renderPlot({
        ...
        dat_slope(first_straight_section(data_ok(), end_t()))
  ...
  observeEvent(input$run_process , {
    ...
    if (input$cores_to_use > 1) {
      ...
      Gwinput = list(Vi = c(8.333333e-02,-3.208333e+01,1.279000e+03,-1.562367e+04,8.424417e+04,-2.369575e+05,3.759117e+05,
                         -3.400717e+05,1.640625e+05,-3.281250e+04), Cbar = -0.5, Skin = -0.5)
    ...
    fitted_data <- data.frame(time = DimlesTime[1:end_t()], drawdown = sim)
    resist <- (data_to_report()$Q / (2 * pi * data_to_report()$Trans)) * result$Skin
    well_stor <- result$Cbar * 2 * pi * data_to_report()$Stor * data_to_report()$R^2
  ...
  observeEvent(input$show_report, {
    ...
    additional_from_slope(additional_resistances_from_slope(data_to_report()$Trans, dat_slope()$coefficients[2],
                                                            data_to_report()$Q, data_to_report()$Cbar))
  ...
  observeEvent(c(input$next_to_param_xlsx, input$next_to_param_txt, input$next_proj1t),  {

   if(input$next_to_param_xlsx == 0 && input$next_to_param_txt == 0 && input$next_proj1t == 0)
     return()
  ...
  observeEvent(c(input$param_to_eval, input$aqua_thick, thickness(), input$show_report), {
  ...
  onStop(function() {
    to_del = c("cores", "packages", "set_cores", "ipak", "setupWT", "obsSnorm", "Gwinput")

    for(i in 1:5)
    {
      if(exists(to_del[i]))
        remove(list = to_del[i], pos = 1, inherits = TRUE)
    }
    })

  observeEvent(c(input$navbar, input$show_report), {
    nav_change(0)

    if(input$navbar == "Report")
    {
      output$style_taggg <- renderUI({
        return(tags$head(
    ...
    else
    {
      ...
      output$style_tagg <- renderUI({
        return(tags$head(
          tags$style(HTML("body{background-image: url( 'background.jpg' ); background-size: 100% !important; opacity: 1;
